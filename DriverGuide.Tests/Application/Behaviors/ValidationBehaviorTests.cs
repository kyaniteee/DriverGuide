...
public class ValidationBehaviorTests
{
    ...
    {
        var validators = Enumerable.Empty<IValidator<LoginUserQuery>>();
        var behavior = new ValidationBehavior<LoginUserQuery, string>(validators);
        var query = new LoginUserQuery { Login = "test", Password = "password" };

        var result = await behavior.Handle(query, _next, CancellationToken.None);
    ...
    {
        var validator = Substitute.For<IValidator<LoginUserQuery>>();
    ...
    public async Task Handle_InvalidRequest_ShouldThrowValidationException()
    {
        ...
        var validators = new[] { validator };
        var behavior = new ValidationBehavior<LoginUserQuery, string>(validators);
        var query = new LoginUserQuery { Login = "", Password = "password" };

        await Assert.ThrowsAsync<ValidationException>(
            () => behavior.Handle(query, _next, CancellationToken.None));
    ...
    {
        var validator1 = Substitute.For<IValidator<LoginUserQuery>>();
        ...
        var validator2 = Substitute.For<IValidator<LoginUserQuery>>();
    ...
    {
        var validatorPass = Substitute.For<IValidator<LoginUserQuery>>();
        validatorPass.ValidateAsync(Arg.Any<IValidationContext>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(new ValidationResult()));

        var validatorFail = Substitute.For<IValidator<LoginUserQuery>>();
        ...
        var behavior = new ValidationBehavior<LoginUserQuery, string>(validators);
        var query = new LoginUserQuery { Login = "validuser", Password = "short" };
